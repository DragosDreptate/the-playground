...
| Découverte publique | oui (annuaire) | non | **oui** (Explorer) |
...
2. Créer des **événements** dans cette Communauté (créés en Brouillon, publiés quand prêts)
...
> - **FR** : Circle → **Communauté** (féminin : une Communauté, cette Communauté), Moment → **événement** (masculin : un événement, cet événement, Brouillon, Publié, Annulé, Passé), Host → Organisateur, Player → Participant, Register → **S'inscrire**, Dashboard → **Mon espace**, Explorer → **Explorer**
> - **EN** : Circle → **Community**, Moment → **Event**, Host reste inchangé. Player → **Member**, Register → **Join**, Dashboard → **Dashboard**, Explorer → **Explore**
>
> **Distinction "Rejoindre" vs "S'inscrire" (FR)** — volontairement non unifiée :
> - **Rejoindre** = action sur un **Circle** (adhésion durable à une communauté : "Rejoindre", "Quitter", "Membre")
> - **S'inscrire** = action sur un **Moment** (RSVP ponctuel à un événement : "S'inscrire", "Annuler mon inscription", "Inscrit")
> - En EN, les deux contextes utilisent **Join** (le verbe anglais couvre naturellement les deux). Ne pas chercher à unifier en FR.
...
- Pas de feed social
- Pas de marketplace (mais Explorer : annuaire simple de Communautés publiques, filtrable par thème/localisation)
...
### Focus et sélection des champs — jamais de rose
...
>
> **Sélection de texte neutre** : ne pas surligner la sélection de texte en rose (`selection:bg-primary` / `selection:text-primary-foreground`) sur un champ. Laisser la sélection par défaut du navigateur (neutre), cohérente avec le `textarea`.
>
...
- Statut Brouillon (DRAFT) par défaut à la création — publication explicite via bouton "Publier"
- Liste d'attente
- Export CSV des inscrits
- Communication directe avec les Participants inscrits (email groupé depuis le tableau de bord de l'événement)
- Pièces jointes aux événements (PDF, JPEG, PNG, WebP — 3 fichiers max, 10 Mo chacun)
- Co-organisateurs (promotion / rétrogradation de membres)
- Invitations directes à une Communauté (par email ou lien privé)
- Validation manuelle des inscriptions (événement et/ou Communauté)
- Paiements via Stripe Connect (événements payants)
- Widget d'intégration sur site externe
- Radar concurrentiel (détection conflits de créneaux)
- Assistant IA basique : non implémenté (Radar IA existant en mode lab)
...
- Paiement Stripe si nécessaire (pour les événements payants)
- Notifications email (confirmation, rappels 24h, changements, annulations)
- Fil de commentaires sur l'événement (avec pièces jointes photos)

### Plateforme
- Explorer : répertoire public de Communautés (filtrable thème/localisation, tri Recommandé/Popularité/Date, section "À la une")
- Stripe Connect pour événements payants
- Réseaux de Communautés (regroupement de plusieurs Communautés)
- Admin plateforme : tableau de bord, gestion utilisateurs/Communautés/événements/réseaux
...
| `pnpm db:push:staging` | Push le schema vers la DB **staging** (avec confirmation) |
| `pnpm db:studio` | Ouvre Prisma Studio (UI de visualisation DB) |
| `pnpm db:studio:prod` | Ouvre Prisma Studio sur la DB **production** |
| `pnpm db:studio:staging` | Ouvre Prisma Studio sur la DB **staging** |
...
| `pnpm db:backfill-public-id` | Backfill des identifiants publics (dev) — génère `publicId` pour les utilisateurs qui n'en ont pas |
...
| CircleMembership | `circle.ts` | Relation User ↔ Circle avec rôle (HOST / CO_HOST / PLAYER) |
| Moment | `moment.ts` | Événement individuel, page autonome partageable |
| Registration | `registration.ts` | Inscription User à un Moment (+ liste d'attente, paiement) |
| Comment | `comment.ts` | Commentaire sur un Moment |

### Schema Prisma (`prisma/schema.prisma`)

Inclut les modèles domaine + modèles Auth.js (Account, Session, VerificationToken) + modèles infrastructure (RadarUsage, RateLimit, CircleNetwork, CircleNetworkMembership, SiteSettings). Tables en snake_case via `@@map`.
...
- **MomentStatus** : `DRAFT` (brouillon, défaut à la création) → `PUBLISHED` (transition sens unique via `publishMoment`) → `PAST` (automatique) / `CANCELLED`
