#!/usr/bin/env bash
# ─────────────────────────────────────────────
# db:backfill-notification-prefs:prod
# Met à true les préférences de notifications
# pour tous les users existants en production.
#
# Usage: pnpm db:backfill-notification-prefs:prod
# ─────────────────────────────────────────────
...
echo ""
echo "⚠️  Vous êtes sur le point de mettre à jour les préférences de notifications en PRODUCTION."
echo "   Branch: production (${NEON_PROD_BRANCH_ID})"
echo "   Action : notifyNewRegistration, notifyNewComment, notifyNewFollower, notifyNewMomentInCircle → true"
echo "   (Uniquement pour les users qui ont au moins une préférence à false)"
echo ""
...
echo ""
echo "🌱 Backfill des préférences de notifications en production..."
DATABASE_URL="${PROD_URL}" npx tsx scripts/backfill-notification-prefs.ts
