#!/usr/bin/env bash
# ─────────────────────────────────────────────
# db:seed-demo-data:prod — Injecte les données
# de démo dans la branche Neon production.
#
# Usage: pnpm db:seed-demo-data:prod
# ─────────────────────────────────────────────
...
echo ""
echo "⚠️  Vous êtes sur le point d'injecter des données de démo en PRODUCTION."
echo "   Branch: production (${NEON_PROD_BRANCH_ID})"
echo "   Les utilisateurs auront le domaine @demo.playground (facilement identifiables et supprimables)."
echo ""
...
echo ""
echo "🌱 Injection des données démo en production..."
DATABASE_URL="${PROD_URL}" npx tsx scripts/db-seed-demo-data.ts
