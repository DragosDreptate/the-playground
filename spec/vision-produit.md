# The Playground — Vision produit

> Document de référence pour la rédaction marketing, les campagnes email, les pitch produit,
> les présentations investisseurs et le contenu éditorial.
>
> Tout ce qui définit le produit, son positionnement, son univers et sa proposition de valeur
> est ici. Ce document est la source unique pour tous les supports de communication.
...
**The Playground** — l'espace de jeu des communautés.

Le nom évoque un terrain ouvert, un lieu de rencontre, un espace d'expression. Pas une vitrine, pas une marketplace — un terrain de jeu où chaque communauté invente ses propres règles.
...
### Baseline courte (pitch en une phrase)

**FR** : La plateforme gratuite pour créer votre communauté et organiser des événements mémorables.

**EN** : The free platform to build your community and host unforgettable events.

### Elevator pitch (30 secondes)

> The Playground, c'est le modèle Meetup avec l'expérience Luma, 100% gratuit.
>
> Aujourd'hui, un organisateur de communauté a le choix entre des outils datés et payants (Meetup) ou des plateformes design mais sans rétention (Luma). On perd ses participants après chaque événement.
>
> The Playground résout ça : chaque événement est une page premium partageable (comme Luma), mais chaque inscription transforme un participant ponctuel en membre d'une communauté durable (comme Meetup). Sans abonnement, sans commission.
>
> Résultat : les organisateurs construisent une audience qu'ils conservent, et les participants trouvent des communautés qui durent.

---

## 2. Problème

### Ce que vivent les organisateurs aujourd'hui

Les communautés bricolent. Elles jonglent entre :

- **Meetup** pour la visibilité — mais le design est daté, les prix ont triplé, et l'export des données est bloqué
- **Luma** pour le design — mais chaque événement est une île : pas de communauté, pas de membres, les gens repartent et il faut recommencer à zéro
- **Google Forms + Eventbrite + WhatsApp + tableurs** — pour tout le reste, en empilant les outils

Le résultat : des audiences éphémères, des données dispersées, du temps perdu, et zéro continuité entre deux événements.

### Le problème fondamental

**Les plateformes existantes sont event-centric.** L'événement est le produit final. Après l'événement, il ne reste rien. Pas de groupe, pas de membres, pas de lien. L'organisateur doit repartir de zéro à chaque fois — recruter, convaincre, inscrire.

Ce modèle détruit la valeur la plus précieuse d'un organisateur : **sa communauté**.

### Les douleurs spécifiques

| Douleur | Qui | Intensité |
|---------|-----|-----------|
| Je perds mes participants après chaque événement | Organisateur | Critique |
| Je dois payer pour organiser (abo Meetup, fees Eventbrite/Luma) | Organisateur | Forte |
| Je ne peux pas exporter mes données (lock-in Meetup) | Organisateur | Forte |
| Je dois empiler 5 outils pour gérer ma communauté | Organisateur | Forte |
| Le design de ma page événement est daté (Meetup) | Organisateur | Moyenne |
| Je ne retrouve pas mes événements / communautés facilement | Participant | Moyenne |
| L'inscription est laborieuse (Meetup : compte obligatoire, formulaire long) | Participant | Moyenne |
...
**The Playground est la plateforme community-centric qui combine le modèle communautaire de Meetup, l'expérience premium de Luma, et la gratuité totale.**

### Le modèle : community-centric, pas event-centric

C'est la différence structurelle. Tout le produit est construit autour d'une conviction : **la communauté est plus importante que l'événement.**

```
Luma (event-centric) :
  Événement → Inscription → L'événement a lieu → Fin.
...
L'événement n'est pas le produit final — c'est la **porte d'entrée** vers une communauté durable.
...
## 4. Univers sémantique

Le vocabulaire de The Playground est structurant. Il porte le positionnement et crée un univers cohérent.
...
**Construisez une audience qui vous appartient — pas à la plateforme.**
...
**5. Tout en un**
Un seul outil pour gérer votre communauté, organiser vos événements, communiquer avec vos membres et suivre votre audience. Plus besoin d'empiler Meetup + Google Forms + Mailchimp + Excel.
...
```
    ...
    ──────────────────────+──────────────────────→ UX premium
                          |
         Eventbrite ●     |          ● Luma
         (billetterie,    |           (design top,
          fees élevées)   |            pas de communauté)
                          |
                    Event-centric
```

### Tableau comparatif (pour pitch / landing page)
...
**Bending Spoons** (propriétaire d'Evernote, WeTransfer) a racheté **Meetup** (2024) puis **Eventbrite** (2025). Pattern connu : acquisition → licenciements massifs → hausse de prix → extraction maximale.

Meetup a triplé ses prix en 2024. Les organisateurs migrent. La fenêtre d'opportunité est de **12-18 mois**.

---

## 7. Parcours utilisateur — Les moments clés
...
### L'Organisateur débutant — "Le Créateur qui se lance"

**Qui** : animateur de communauté tech, responsable asso sportive, organisateur de meetups, fondateur de groupe hobby. Il a une petite audience (20-100 personnes) et utilise actuellement WhatsApp + Google Forms + Meetup.

**Ce qu'il cherche** : un outil simple pour organiser ses événements sans payer, avec un design qui impressionne sa communauté.
...
**Qui** : organisateur régulier (2+ événements/mois), communauté établie (100-1000 membres). Souvent en train de migrer depuis Meetup (prix) ou Luma (manque de rétention).

**Ce qu'il cherche** : efficacité dans la gestion, fidélisation de sa communauté, données exploitables.
...
**Qui** : personne qui reçoit un lien d'événement sur mobile, souvent via un ami ou un réseau social. Ne connaît pas la plateforme.
...
### Évolution : Plan Pro (post-MVP)

| Feature Pro | Description |
|-------------|-------------|
| Analytics avancés | Taux de présence, rétention, engagement, insights |
| Branding personnalisé | Logo, couleurs, domaine custom |
| IA avancée | Analyse no-show, suggestions de créneaux, génération de contenu |
| API complète | Lecture + écriture, intégrations custom |
| Multi-canal | SMS, push, WhatsApp (en plus de l'email) |

Le Plan Pro est un upsell pour les Organisateurs qui veulent aller plus loin — pas un prérequis pour utiliser la plateforme.
...
Chaque décision produit est prise à travers le prisme communautaire. L'événement est un moyen, pas une fin.
...
Moins de réglages = meilleure adoption. L'essentiel d'abord (titre, date, lieu, description), les options avancées masquées.

### 4. Mobile-first

Le parcours Participant est optimisé pour mobile. Le lien arrive par WhatsApp → il ouvre sur mobile → il s'inscrit sur mobile. Chaque page doit être parfaite sur un écran de 375px.
...
Les données appartiennent à l'Organisateur, pas à la plateforme. Export complet à tout moment. Pas de lock-in, pas de piège.
...
### Phase 1 — Lancement : France

- Communautés tech (meetups, user groups, hackathons)
- Communautés créatives (design, art, musique)
- Communautés sport & bien-être (running clubs, yoga, randonnée)
- Associations et groupes locaux
- Communautés entrepreneuriat (startups, founders, networking)

**Stratégie d'amorçage** : cibler les communautés tech françaises mécontentes de Meetup. Dogfooding immédiat avec des communautés proches.

### Phase 2 — Expansion : Europe

Même cible, extension géographique. L'architecture i18n (FR/EN dès V1) facilite l'expansion.

### Phase 3 — Scale : International

Focus sur les marchés où Meetup est dominant et contesté.

---

## 12. Éléments de langage — Boîte à outils

### Accroches (pour campagnes, emails, réseaux)
...
### Arguments par objection

| Objection | Réponse |
|-----------|---------|
| "J'utilise déjà Meetup" | Meetup a triplé ses prix et bloque l'export de vos données. The Playground est gratuit, vos données vous appartiennent. |
| "J'utilise déjà Luma" | Luma est event-centric : après l'événement, vos participants disparaissent. The Playground transforme chaque participant en membre de votre communauté. |
...
## 13. Synthèse visuelle (pour slides / pitch deck)

### Slide 1 — Le problème

> Les plateformes d'événements sont event-centric. Après l'événement, il ne reste rien.
> L'organisateur perd ses participants et recommence à zéro.
...
> Le modèle Meetup (communautés durables)
> + L'expérience Luma (design premium, friction zéro)
> + 100% gratuit (zéro commission, zéro abonnement)

### Slide 5 — Le marché

> Meetup racheté par Bending Spoons → prix x3, exode des organisateurs.
> Eventbrite racheté par le même acquéreur → consolidation du marché.
> Fenêtre d'opportunité de 12-18 mois pour une alternative ouverte et gratuite.

### Slide 6 — Le CTA

> Lancez votre communauté. C'est gratuit, pour toujours.
