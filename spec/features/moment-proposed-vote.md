...
Pour les autres statuts (`DRAFT`, `PUBLISHED`), la refonte complète du combobox d'édition actuel vers des boutons d'action contextuels est **hors scope de cette spec**. Voir l'entrée "Refonte UX édition d'événement" dans le BACKLOG. Dans le cadre de cette PR, s'assurer simplement que :
...
**Multi-Organisateurs** : si la Communauté a plusieurs `HOST` (cf. feature future Co-Organisateurs), tous voient la même vue Organisateur et peuvent tous agir sur les 3 boutons. Pas de verrouillage pessimiste. **Premier qui agit gagne** : si un second HOST tente une action alors que le statut a déjà changé (ex. tentative de publier un Moment déjà publié par un autre HOST), le usecase serveur renvoie l'erreur correspondante (`MomentNotProposedError`), traduite côté UI par un message explicite ("Cet événement a déjà été publié par un autre Organisateur") et un rechargement de la page.
