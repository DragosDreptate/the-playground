...
- **Personnalisation prénom** (révisée 2026-06-10, post-livraison) : ~~salutation automatique en tête du corps~~ remplacée par un **placeholder insérable**. La salutation auto était invisible pour l'Organisateur (risque de double salutation « Bonjour Alice, Salut à tous, »). À la place : un bouton « Prénom » dans la barre d'outils insère `{prénom}` (FR) / `{firstName}` (EN) au curseur ; un hint sous l'éditeur explique le remplacement. Résolution par destinataire dans l'adapter (`resolveFirstNamePlaceholders`, alias acceptés : `{prénom}`, `{prenom}`, `{firstName}`, `{first name}`, insensible à la casse) : prénom échappé HTML, ou token retiré avec son espace précédente si le prénom est inconnu (« Bonjour {prénom}, » → « Bonjour, »). L'Organisateur voit exactement ce qui partira.
