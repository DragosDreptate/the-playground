...
**L'annulation est portée par un usecase/action dédié `cancelMoment`** : il rembourse les inscrits payants, rejette les inscriptions en attente d'approbation, passe en CANCELLED, puis l'action envoie l'email d'annulation aux inscrits. La notification au `delete` est restreinte à un **filet de sécurité** (suppression directe d'un PUBLISHED, chemins hors-UI).
