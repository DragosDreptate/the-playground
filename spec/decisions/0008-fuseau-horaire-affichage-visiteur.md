...
2. **Un fuseau horaire est stocké par événement** (nouveau champ `timezone` sur `Moment`, identifiant IANA type `Europe/Dublin`), capté **silencieusement depuis le navigateur de l'organisateur, à la création uniquement**. Pas de sélecteur : le fuseau du navigateur est le bon dans la quasi-totalité des cas, et un champ de plus dans le formulaire de création irait contre le principe de minimalisme. Corollaire impératif : le fuseau **n'est jamais recalculé à l'édition** — sans sélecteur, un écrasement silencieux (organisateur éditant depuis un autre fuseau) serait sans recours pour lui.
...
- **Négatives / coûts** :
  ...
  - **Fuseau non corrigeable depuis l'app** (conséquence du « zéro sélecteur ») : un organisateur qui crée depuis un fuseau qui n'est pas celui de son événement (voyage, machine mal réglée) enregistre un fuseau faux et ne peut ni le voir clairement, ni le corriger. Atténuation : le formulaire affiche le fuseau retenu sous les créneaux (badge « GMT+1 Dublin »), ce qui rend l'écart visible à qui y prête attention ; la correction, elle, passe par une intervention en base. À rouvrir si le cas se présente plus d'une fois.
