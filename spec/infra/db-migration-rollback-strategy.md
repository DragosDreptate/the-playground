...
## Validation du titre d'un événement (sujet connexe)

La limite de 200 caractères sur le titre d'un événement est actuellement **uniquement côté client** (`maxLength={200}` sur l'input HTML). Elle peut être bypassée via un appel direct à la Server Action.
...
- [ ] Validation titre événement dans les usecases (max 200 chars)
