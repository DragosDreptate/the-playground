...
| #007 | medium | evol | Thèmes Communauté en collection de tags (curés + libres) | `category` (enum unique) + `customCategory` capture mal la richesse réelle d'une Communauté multi-sujets, et empêche la découverte par thème fin. | Remplacer par une collection de tags : vocabulaire curé par la plateforme (autocomplete) + tags libres autorisés. Impacts : schema Prisma, UI création/édition Communauté, filtres Explorer, migration des catégories existantes. | à créer |
