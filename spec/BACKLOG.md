...
| #005 | medium | feature | Contacter l'Organisateur depuis la page Communauté / événement | Pas de canal direct pour poser une question à l'Organisateur avant inscription, le Participant passe par des workarounds (commentaire public, inscription forcée). Réf. Luma : [screenshot](assets/backlog/luma-event-reference.png) (lien "Contacter l'organisateur" colonne gauche). | Bouton "Contacter" sur pages Communauté et événement (authentifié requis). Envoi via `EmailService`, `reply-to` = email du Participant. | — |
...
| #007 | medium | evol | Thèmes Communauté en collection de tags (curés + libres) | `category` (enum unique) + `customCategory` capture mal la richesse réelle d'une Communauté multi-sujets, et empêche la découverte par thème fin. Réf. Luma : [screenshot](assets/backlog/luma-event-reference.png) (chips "# IA", "# Arts et Culture"). | Remplacer par une collection de tags : vocabulaire curé par la plateforme (autocomplete) + tags libres autorisés. Impacts : schema Prisma, UI création/édition Communauté, filtres Explorer, migration des catégories existantes. | à créer |
...
| #009 | low | feature | Rappel 1h avant événement | Participants loupent les événements malgré le rappel 24h. | Réutiliser l'infra cron 24h, champ `reminder1hSentAt`, fenêtre 50-70min. | — |
| #010 | low | chore | Stratégie migrations DB + rollback prod | `db:push` peut silencieusement supprimer des données en prod (drop+recreate sur renommage). | Passer à `prisma migrate` + snapshot Neon + PITR comme filet. | `spec/infra/db-migration-rollback-strategy.md` |
| #011 | low | chore | Rate limiting des actions sensibles | Aucune protection anti-abus sur les server actions (inscription, création). | Upstash Rate Limit. Cibles : 10 inscriptions/min/IP, 5 créations/heure/user. | — |
| #012 | low | feature | Export données Organisateur étendu | Export CSV par événement existe, mais pas à l'échelle Communauté (membres, historique, inscrits cumulés). | Ajouter un export CSV Communauté. | — |
| #013 | low | feature | Stats Communauté basiques | L'Organisateur n'a pas de métriques de santé (tendance membres, taux de remplissage). | Bloc stats sur la page Communauté Organisateur. | — |
| #014 | low | fix | OAuth Google dans les navigateurs in-app | Google refuse les WebViews (`Error 403: disallowed_useragent`), Instagram/WhatsApp/FB bloqués. | Détecter le user-agent, afficher un CTA "ouvrir dans mon navigateur" sur `/auth/error`. | — |
| #015 | low | chore | CI : tests d'intégration | Les tests d'intégration (repositories, services) ne tournent pas en CI. | Job dédié avec service PostgreSQL GitHub Actions. | — |
| #016 | low | chore | Test unitaire `joinCircleDirectly` | Fichier de test dédié manquant. | Ajouter `__tests__/joinCircleDirectly.test.ts`. | — |
| #017 | low | chore | E2E : rejoindre une Communauté directement | Parcours `JoinCircleButton` (sans événement) non couvert en E2E. | Nouveau spec Playwright basé sur l'infra existante. | — |
| #018 | low | fix | Page `/changelog` bilingue | `CHANGELOG.md` rédigé en FR uniquement, la page EN affiche du FR. | Deux fichiers FR/EN, ou accepter le FR partout (contenu technique). | — |
