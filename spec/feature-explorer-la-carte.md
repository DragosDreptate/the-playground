...
- N membres · N événements à venir
- Teaser prochain événement : titre + date
- CTA : lien vers `/circles/[slug]` (page Communauté publique)
...
- En-tête : nom de la Communauté (prominent) + badge catégorie
- Thumbnail gradient événement
- Titre de l'événement
- Date + heure · lieu ou "En ligne"
- N inscrits (+ places restantes si capacité définie)
- Ville de la Communauté en texte secondaire si renseignée
...
**Contenu** (similaire à la page Communauté dashboard, sans les contrôles Organisateur) :
- Layout 2 colonnes (identique au design existant)
- Description, Organisateurs, stats (membres, événements)
- Timeline des événements à venir (cliquables → `/m/[slug]`)
- Pas de bouton Modifier/Supprimer
- CTA : "S'inscrire à cette Communauté" → s'inscrit au prochain événement ou demande à rejoindre

**Note** : cette page peut réutiliser `MomentTimelineItem` existant (sans les liens dashboard).

---

## Formulaire Communauté — Champs à ajouter
