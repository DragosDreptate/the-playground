...
export default async function EditMomentPage({
  ...
  const [initialAttachments, venues] = await Promise.all([
    prismaMomentAttachmentRepository.findByMoment(moment.id),
    prismaCircleVenueRepository.findByCircleId(circle.id),
  ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <MomentForm moment={moment} circleSlug={slug} circleName={circle.name} circleDescription={circle.description ?? undefined} circleCoverImage={circle.coverImage} stripeConnectActive={!!circle.stripeConnectAccountId} priceLocked={priceLocked} initialAttachments={initialAttachments} venues={venues} action={boundAction} />
    </div>
  );
}
