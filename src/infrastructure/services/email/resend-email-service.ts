...
export function createResendEmailService(): EmailService {
  ...
  return {
    ...
    async sendHostNewCircleMember(
      data: HostNewCircleMemberEmailData
    ): Promise<void> {
      await send({
        from,
        to: data.to,
    ...
    async sendRegistrationRemovedByHost(
      data: RegistrationRemovedByHostEmailData
    ): Promise<void> {
      await send({
        from,
        to: data.to,
        subject: data.strings.subject,
        react: RegistrationRemovedByHostEmail({ ...data, baseUrl }),
      });
    },
