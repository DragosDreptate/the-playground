...
export function AddToCalendarMenu({ data, appUrl }: Props) {
  const t = useTranslations("Moment.public.addToCalendar");

  const googleUrl = buildGoogleCalendarUrl(data, appUrl, {
    join: t("calendarJoin"),
    organizedBy: t("calendarOrganizedBy"),
  });
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          size="sm"
          aria-label={t("label")}
          className="shrink-0 max-lg:size-9 max-lg:p-0"
        >
          <CalendarPlus className="size-4" />
          <span className="hidden lg:inline">{t("label")}</span>
          <ChevronDown className="text-muted-foreground hidden size-3.5 lg:inline" />
        </Button>
      </DropdownMenuTrigger>
