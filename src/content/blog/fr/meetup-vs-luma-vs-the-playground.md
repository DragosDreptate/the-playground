...
Avant de comparer les fonctionnalités, il faut comprendre une différence structurelle entre ces plateformes.

**Certaines sont centrées sur l'événement** : vous créez une page, des gens s'inscrivent, l'événement a lieu, et chacun repart de son côté. Si vous voulez organiser un autre événement, vous repartez de zéro.
...
| --- | --- | --- | --- |
| **Modèle** | Communauté | Communauté | Événement (+ Calendar) |
| **Gratuit pour l'organisateur** | Oui | Variable (voir ci-dessous) | Oui |
| **Commission événements payants** | 0% (frais Stripe seuls) | Via plateforme | 5% (ou 0% à 59$/mois) |
| **Page événement premium** | Oui | Non | Oui |
| **Inscription sans compte** | Non (magic link ou OAuth) | Non (compte obligatoire) | Oui (email seul) |
| **Page communauté publique** | Oui (membres, événements, identité) | Oui (groupe) | Partielle (Calendar, sans membres) |
| **Export des participants** | CSV par événement | CSV membres + API restreinte | CSV par événement + contacts Calendar |
| **Découverte publique** | Oui (annuaire) | Oui (annuaire, le plus large) | Oui (Discover, 50+ villes) |
...
- **Le modèle communautaire** : groupe persistant, membres, notifications. C'est la référence.
- **La découverte locale** : l'annuaire par ville et thème reste le plus complet du marché. Si vous lancez un nouveau groupe, Meetup vous donne de la visibilité organique.
- **La base installée** : des millions d'utilisateurs actifs dans le monde.

### Les limites

- **Le double paywall** : organisateurs et membres paient. Pour une communauté gratuite, c'est un frein réel à l'adoption.
- **L'interface** : remaniée après le rachat, mais encore loin du standard Luma. L'inscription nécessite un compte complet (email, mot de passe, profil), ce qui freine la conversion depuis un lien partagé.
- **L'export** : CSV des membres disponible pour les organisateurs. L'API publique a été fortement restreinte ces dernières années.
...
Luma est le bon choix si vous organisez des **événements ponctuels ou des conférences** où la qualité de la page d'inscription est déterminante et où la communauté persistante n'est pas un enjeu. C'est aussi le meilleur outil si vous avez besoin de communications multi-canal (SMS, WhatsApp) dès maintenant.

## The Playground : la communauté d'abord, 100% gratuit

The Playground part d'un constat simple : Meetup a le bon modèle (communauté persistante) mais un produit vieillissant et un modèle payant. Luma a la bonne expérience mais pas de couche communautaire. The Playground combine les deux.
...
- **La communauté persistante** : chaque communauté a sa page publique avec ses événements (à venir et passés), ses membres et son identité. C'est la couche de rétention que Luma n'a pas.
- **L'inscription automatique** : s'inscrire à un événement = rejoindre la communauté. Aucune étape supplémentaire, zéro friction.
- **La social proof** : les inscrits sont visibles sur la page événement (avatars, nombre de places restantes). Ça aide à la conversion.
- **Les événements payants via Stripe Connect** : l'organisateur reçoit le paiement directement, sans intermédiaire ni commission plateforme.
...
- **Pas de SMS/WhatsApp** dans les notifications (pour l'instant, email uniquement).
- **Plateforme jeune** : moins de fonctionnalités avancées que Luma (pas de check-in QR, pas de communications multi-canal, pas de récurrence native encore).
...
Il n'y a pas de "meilleur outil" universel. Le choix dépend de ce que vous construisez :

- **Vous misez sur la découverte organique** et vous acceptez le coût → **Meetup**
